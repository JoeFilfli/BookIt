...
async function getServiceWithBusiness(serviceId: string) {
...
// POST /api/services/[serviceId]/schedules — Add a schedule block
export async function POST(
  ...
  try {
    ...
    const service = await getServiceWithBusiness(serviceId);
    if (!service) {
    ...
    await requireBusinessOwner(service.businessId);
    ...
    const schedule = await prisma.serviceSchedule.create({
      data: { serviceId, dayOfWeek, startTime, endTime },
    });
...
// GET /api/services/[serviceId]/schedules — List schedule blocks
export async function GET(
  ...
  try {
    ...
    console.error("List service schedules error:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
